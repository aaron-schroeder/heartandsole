...
class RunPower(object):
  """A tool for calculating running power metrics.

  Construction of a RunPower object creates a pandas DataFrame object
  with point-to-point speed and grade data.
  """

  def __init__(self, speeds_ms, grades):
    """Creates a RunPower object from a list of speeds and grades.

    Args:
      speeds_ms: An array-like object representing point-to-point 
                 speeds in m/s.
      grades: An array-like object representing point-to-point 
              decimal grades.
    """
    if type(speeds_ms) == pandas.DataFrame:
      speeds_ms = speeds_ms.values.squeeze()
    
    if type(grades) == pandas.DataFrame:
      grades = grades.values.squeeze()

    rows = [[speed_ms, grade] for speed_ms, grade in 
            zip(speeds_ms, grades)]
  ...
  def _c_r(speed, grade):
    ...
    See the documentation for RunPower.power for information
  ...
  def _calc_power(pace):
    ...
    return RunPower._c_r(speed, 0.0) * speed
  ...
  def power(self):
    ...
    if 'power' not in self.data.columns:
      ...
      c_r = self._c_r(self.data['speed'], self.data['grade'])
      ...
      self.data['power'] = c_r_smooth * self.data['speed']
      #self.data['power'] = fitanalysis.util.moving_average(
      #    c_r_smooth * self.data['speed'], 120)

    return self.data['power'].to_numpy().squeeze()
