...
def elevation_smooth(distances, elevations, window_length=3, polyorder=2):
  ...
  with warnings.catch_warnings():
    ...
    data_ds = elevations.groupby(idx).apply(np.median).interpolate(limit_direction='both').to_frame()
...
def grade_smooth(distances, elevations):
  ...
  return (elevations_smooth.diff() / distances.diff()).to_numpy()
...
def grade_raw(distances, elevations):
  ...
  return (elevations.diff() / distances.diff()).to_numpy()
